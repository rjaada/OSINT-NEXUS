...
export function AiAnalyst() {
  ...
  const fetchReport = async (force = false) => {
    setLoading(true)
    try {
      const res = await fetch(`${API_BASE}/api/analyst${force ? "?force=true" : ""}`, { credentials: "include", cache: "no-store" })
      if (res.ok) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-[10px] font-bold tracking-[0.2em] text-[#e0e0e8] uppercase flex-1 text-left">
          AI Crisis Analyst
        </span>
        {!hasKey && (
          <span title="Groq API key not configured">
            <AlertTriangle className="h-3 w-3 text-osint-amber" />
          </span>
        )}
        <span
          className={`text-[8px] font-bold tracking-widest px-2 py-0.5 rounded ${isCritical ? "animate-blink" : ""}`}
      ...
      {expanded && report && (
        <div className="px-4 pb-4 space-y-3 border-t border-white/[0.05]">
          {/* Summary */}
          <p className={`text-[11px] leading-relaxed pt-3 ${isCritical ? "text-[#d0b0f0]" : "text-[#a0a0c0]"}`}>
          ...
          <div className="flex items-center justify-between text-[8px] text-muted-foreground pt-1">
            ...
            {level === "CRITICAL" && (
              <span className="text-[8px] text-osint-purple font-bold tracking-wider animate-blink">
                ⚠ ESCALATED
              </span>
            )}
          </div>
