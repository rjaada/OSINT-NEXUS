...
export default function ArabicAdminUsersPage() {
  ...
  const setUserRole = async (username: string, nextRole: Role) => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}))
      if (!res.ok) {
        setMsg(data?.detail || "فشل تحديث الدور")
        return
      }
      ...
      <main className="px-4 md:px-6 py-5">
        <div className="max-w-6xl mx-auto">
          <header className="mb-4">
            ...
            <h1 className="text-2xl md:text-3xl font-semibold">إدارة أدوار المستخدمين</h1>
            <p className="text-xs text-muted-foreground mt-2">إدارة صلاحيات viewer و analyst و admin.</p>
          </header>
          ...
          {role !== "admin" ? (
            ...
            <section className="rounded-lg border border-white/10 bg-black/30 p-3">
              <div className="flex items-center gap-2 mb-3 text-[11px]">
                ...
                <span className="ml-2 text-muted-foreground">عدد المدراء:</span>
                <span className="text-osint-blue">{adminsCount}</span>
                <button
                  className="mr-auto text-[10px] px-2 py-1 rounded border border-osint-blue/40 text-osint-blue"
                ...
                {users.map((u) => (
                  <article key={u.username} className="rounded border border-white/10 p-2 grid md:grid-cols-5 gap-2 items-center">
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground">آخر تحديث: {u.updated_at}</p>
                    </div>
                    <p className="text-[11px] text-muted-foreground">الإنشاء: {u.created_at}</p>
                    <p className="text-[11px] text-muted-foreground">الدور الحالي: <span className="text-osint-green">{u.role}</span></p>
