...
export default function V2ArabicHealthPage() {
      ...
      <main className="px-4 md:px-6 py-5">
        <div className="max-w-7xl mx-auto">
          <header className="mb-5">
            <p className="text-[10px] tracking-[0.18em] uppercase text-osint-purple mb-1">التحكم التشغيلي v2</p>
            <h1 className="text-2xl md:text-3xl font-semibold">لوحة صحة النظام والموثوقية</h1>
            <p className="text-xs text-muted-foreground mt-2">مراقبة لحظية للتغذية، الطوابير، المراقب، واتصال PostgreSQL.</p>
          </header>

          <section className="grid md:grid-cols-4 gap-3 mb-4">
            <article className="rounded-lg border border-white/10 bg-black/30 p-3">
              <p className="text-[10px] uppercase tracking-[0.14em] text-osint-green mb-2">النظام</p>
              <p className={data?.status === "nominal" ? "text-osint-green text-sm" : "text-osint-red text-sm"}>{data?.status || "--"}</p>
              <p className="text-[11px] text-muted-foreground mt-1">الاستمرارية: {data ? Math.floor(data.uptime_seconds / 60) : 0}د</p>
            </article>
            <article className="rounded-lg border border-white/10 bg-black/30 p-3">
              <p className="text-[10px] uppercase tracking-[0.14em] text-osint-blue mb-2">المراقب</p>
              <p className="text-sm text-[#d0d0df]">{(data?.watchdog_warnings || []).length}</p>
              <p className="text-[11px] text-muted-foreground mt-1">تحذيرات</p>
            </article>
            <article className="rounded-lg border border-white/10 bg-black/30 p-3">
              <p className="text-[10px] uppercase tracking-[0.14em] text-osint-amber mb-2">Postgres</p>
              <p className={(data?.postgres?.connected ? "text-osint-green" : "text-osint-red") + " text-sm"}>{data?.postgres?.connected ? "متصل" : "غير متصل"}</p>
              <p className="text-[11px] text-muted-foreground mt-1">events_v2: {data?.postgres?.events_count ?? "--"}</p>
            </article>
            <article className="rounded-lg border border-white/10 bg-black/30 p-3">
              <p className="text-[10px] uppercase tracking-[0.14em] text-osint-purple mb-2">التحديث</p>
          ...
          <section className="grid md:grid-cols-3 gap-3 mb-4">
            <article className="rounded-lg border border-white/10 bg-black/30 p-3">
              <p className="text-[10px] uppercase tracking-[0.14em] text-osint-amber mb-2">تنبيهات القواعد</p>
              <p className="text-sm text-[#d0d0df]">{alerts?.total ?? 0}</p>
            </article>
            <article className="rounded-lg border border-white/10 bg-black/30 p-3">
              <p className="text-[10px] uppercase tracking-[0.14em] text-osint-red mb-2">حرج</p>
              <p className="text-sm text-osint-red">{alerts?.critical ?? 0}</p>
            </article>
            <article className="rounded-lg border border-white/10 bg-black/30 p-3">
              <p className="text-[10px] uppercase tracking-[0.14em] text-osint-amber mb-2">تحذير</p>
            ...
            {layerStatus.map((layer) => (
              <article key={layer.key} className="rounded-lg border border-white/10 bg-black/30 p-3">
                ...
                <p className={layer.healthy ? "text-osint-green text-sm" : "text-osint-amber text-sm"}>
                  {layer.healthy ? "سليم" : "متدهور"}
                </p>
                <p className="text-[11px] text-muted-foreground mt-1">آخر نجاح: {layer.lastSuccess || "لا يوجد"}</p>
                <p className="text-[11px] text-muted-foreground">الأخطاء: {layer.errors}</p>
              </article>
            ))}
          </section>
          ...
          <section className="grid md:grid-cols-2 gap-3 mb-4">
            <article className="rounded-lg border border-white/10 bg-black/30 p-3">
              <p className="text-[10px] uppercase tracking-[0.14em] text-osint-blue mb-2">الطوابير</p>
            ...
            <article className="rounded-lg border border-white/10 bg-black/30 p-3">
              <p className="text-[10px] uppercase tracking-[0.14em] text-osint-amber mb-2">تحذيرات المراقب</p>
              <div className="text-[11px] text-muted-foreground space-y-1">
                {(data?.watchdog_warnings || []).length === 0 ? <p>لا يوجد تحذيرات.</p> : data?.watchdog_warnings.map((w) => <p key={w}>• {w}</p>)}
          ...
          <section className="rounded-lg border border-white/10 bg-black/30 p-3 mb-4">
            <p className="text-[10px] uppercase tracking-[0.14em] text-osint-red mb-2">القواعد المفعلة</p>
            <div className="text-[11px] space-y-1">
              {(alerts?.alerts || []).length === 0 ? <p className="text-muted-foreground">لا توجد تنبيهات قواعد نشطة.</p> : (alerts?.alerts || []).map((a, i) => (
          ...
          <section className="rounded-lg border border-white/10 bg-black/30 p-3">
            <p className="text-[10px] uppercase tracking-[0.14em] text-osint-green mb-2">قياسات خام</p>
