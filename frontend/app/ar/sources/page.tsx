      ...
      <main className="px-4 md:px-6 py-5">
        <div className="max-w-7xl mx-auto">
          <header className="mb-5">
            <p className="text-[10px] tracking-[0.18em] uppercase text-osint-blue mb-1">مكتب المصادر</p>
            <h1 className="text-2xl md:text-3xl font-semibold">التحقق من المصدر وصحة التشغيل</h1>
            <p className="text-xs text-muted-foreground mt-2">
              هذا العرض يحتوي حقائق مرصودة واستدلالات نموذجية. الاستدلال ليس دليلاً قاطعًا.
            </p>
          </header>
          ...
          <section className="grid md:grid-cols-3 gap-3 mb-4">
            <article className="rounded-lg p-3 border border-white/10 bg-black/30 md:col-span-2">
              <p className="text-[10px] uppercase tracking-[0.14em] text-osint-green mb-2">حجم المصادر</p>
            ...
            <article className="rounded-lg p-3 border border-white/10 bg-black/30">
              <p className="text-[10px] uppercase tracking-[0.14em] text-osint-amber mb-2">المراقب</p>
              <p className="text-[11px] text-[#c5c5d5]">الحالة: {ops?.status ?? "--"}</p>
              <div className="text-[11px] text-muted-foreground mt-2">
                {(ops?.warnings ?? []).length === 0 ? "لا توجد تحذيرات" : (ops?.warnings ?? []).join(" · ")}
              </div>
            </article>
          </section>
