...
## Session Log — 2026-03-16 (Part 3)

### Plan for this session (IN PROGRESS)

From Research Sweep 7 (Bellingcat + ACLED + ICD 203):

1. **CSP header** — one line in Caddyfile. Last OWASP gap.
2. **ICD 203 confidence mapping** — replace HIGH/MEDIUM/LOW with ICD 203 5-level standard
   - Backend: `reasoning_engine.py` SITREP output + `groq_client.py` trace output
   - Frontend: `intel-feed-v2.tsx` confidence label display
   - Paper defense sentence enabled: "Confidence levels follow ICD 203 analytic standards"
3. **Intel Trace retry button** — add retry UI when trace fails (~50% first-attempt failure rate)
   - `intel-feed-v2.tsx` — retry button in failure state, auto-retry on cold connection
4. **Source tier badge on feed cards** — wire/national/telegram visual indicator
   - `intel-feed-v2.tsx` — small badge: Reuters=WIRE (green), JP=NATL (blue), Telegram=TG (amber)
   - Matches Bellingcat 2-second scan pattern
5. **ACLED notes field + dispute resolution** — when sources conflict, document it
   - `v2_store.py` — add `notes` TEXT column to events_v2
   - `reasoning_engine.py` — `detect_contradictions()` writes "Reuters 12 vs Telegram 40; conservative estimate used"
6. **ingestion.py extraction** ✅ DONE (2026-03-17)
   - `backend/ingestion.py` created (600 lines): geocode_place, call_ollama_json, geolocate_with_ai, geolocate_event, parse_telegram_posts, download_telegram_video, download_video_direct, infer_video_metadata, is_playable_video_url, is_relevant, build_incident_id, should_merge_with_existing, push_event_buffer, ingest_event, _graph_source_id, _sync_event_to_graph_async, and iutils thin-wrappers
   - main.py: 2519 → 2375 lines (-144)
   - All 7 containers healthy, Application startup complete

### Leave to user
- 50 manually labelled eval samples (2–3 hours, user does manually today)

---

## Session Log — 2026-03-17

### What was built (DONE, committed 42035d9 + 3cca6bf)

#### ICD 203 confidence mapping
- `reasoning_engine.py` — `_ICD203_MAP`, `_icd203()` helper, SITREP prompt updated to HIGH/MODERATE/LOW/VERY LOW
- `groq_client.py` — Intel Trace prompt updated to ICD 203, enriches result with `icd203_level` + `icd203_phrase`
- `intel-feed-v2.tsx` — `icd203Label()` helper, confidence badge shows ICD 203 level + phrase

#### Source tier badges
- `intel-feed-v2.tsx` — `sourceTier()` helper maps source to WIRE/NATL/TG/SENSOR badge on every feed card

#### ACLED notes field
- `v2_store.py` — `notes TEXT` column added to events_v2, included in INSERT/UPDATE/SELECT
- `reasoning_engine.py` — `detect_contradictions()` writes ACLED notes field

#### Intel Trace retry button
- `intel-feed-v2.tsx` — retry button shown on trace failure state

#### TypeScript errors fixed
- Pre-existing: `evt.type as string` cast for ALERT/ACTIVITY values not in EventType union
- Pre-existing: `re.source != null` guard for unknown type in related events

#### ingestion.py extraction
- `backend/ingestion.py` — 600 lines, all geocoding/ingest/Telegram helpers extracted from main.py
- main.py: 2519 → 2375 lines

### Bug fixes (same session, committed df0dc78)

#### Empty alerts / empty priority panel / empty news feed
- **Root cause**: `v2_store.py` `SELECT` included `notes` column that didn't exist in the live table (table was created before we added the column). PostgreSQL error on every fetch → all 3 frontend panels empty.
- **Fix 1**: `ALTER TABLE events_v2 ADD COLUMN IF NOT EXISTS notes TEXT DEFAULT ''` — run directly on live DB.
- **Fix 2**: `pollers.py` — moved `last_success["telegram"]` to fire after HTTP 200, not after parsing posts. Prevents watchdog false alarm when channels have no new posts.
- Rebuilt + restarted backend. Application startup complete, all panels restored.

### What is still pending
- 50 manually labelled eval samples (user's task)
- main.py still ~2375 lines — next split target if needed

### Additional work done (committed a07663a)
- LLM context chunking — top 30 events by confidence+recency, sensor 5-min bucketing
- SITREP forward projection — forecast.next_24h / next_72h / next_7d
- Graph temporal decay — 30-day half-life on all Neo4j edges
- Entity disambiguation — IDF/IRGC/Hamas/Hezbollah/Houthis → canonical names
- Bayesian source weights — analyst ratings update SOURCE_RELIABILITY dynamically
- NATO 2×6 display — A/1–F/6 badge on every feed card
...
**1. LLM context chunking** ✅ DONE (2026-03-17)
- `_select_top_events()` picks top 30 by confidence+recency instead of arbitrary first-15
- Sensor timestamp bucketing (5-min) prevents OREF/FIRMS false corroboration inflation
- SITREP prompt now includes `forecast` key: next_24h / next_72h / next_7d forward projection

**2. Content-Security-Policy header** ✅ DONE (already in Caddyfile)
- Was already present on line 33 of Caddyfile — no action needed
...
**9. Dynamic Bayesian source reliability weights** ✅ DONE (2026-03-17)
- `v2_store.get_bayesian_source_weights()` — Bayesian update from analyst star ratings
- Prior weight=5, 5-min cache in main.py `_get_source_weights()`, falls back to static

**10. Graph edge temporal decay** ✅ DONE (2026-03-17)
- `graph_store._decay_weight()` — exponential decay, 30-day half-life
- Applied to all temporal relationships on upsert

**11. Entity disambiguation** ✅ DONE (2026-03-17)
- `graph_store._normalize_entity()` — collapses IDF/Israeli Forces/IRGC/Hamas/Hezbollah/Houthis to canonical names
- Applied to all source nodes on upsert in Neo4j

**12. NATO 2×6 source/claim split display** ✅ DONE (2026-03-17)
- `nato2x6Reliability()` + `nato2x6Credibility()` helpers in intel-feed-v2.tsx
- Every feed card shows `A/1`–`F/6` badge with tooltip (source reliability + claim credibility)
...
## Session Log — 2026-03-17 (Part 2)

### Research Sweep 8 — Five holes + sourced answers (saved to RESEARCH_ANALYSIS.md)
1. Per-pair temporal tolerance (Castanedo 2013) — per-pair dict, not universal constant
2. Cosine similarity for CIB (Stanford IO, EU DisinfoLab) — ≥0.40 binary cosine, length-normalized
3. EWMA baseline (Shewhart + CUSUM) — span=30, z>2/z>3, spike exclusion, 90-obs minimum
4. ACLED civilian targeting structural rule (ACLED §4.3) — Actor 2 classification, not keywords
5. Vague fatality lookup (ACLED sourced minimum) — dozens=12, scores=20, hundreds=100

### What was built this session

#### Per-pair temporal tolerance windows (`backend/reasoning_engine.py`)
- Removed `_bucket_ts()` and `_SENSOR_BUCKET_SEC = 300` universal constant
- Added `_source_type()` helper (oref/firms/adsb/ais/telegram/rss)
- Added `_PAIR_TOLERANCE_SEC` dict: OREF↔FIRMS=180s, OREF↔ADSB=60s, Telegram↔RSS=300s, Telegram↔FIRMS=180s, AIS↔ADSB=120s
- Added `_pair_tolerance(src_i, src_j)` — returns per-pair tolerance in seconds
- `correlate_events()` now uses `dt_sec > window_hours * 3600 + tolerance` instead of bucketed comparison

#### Cosine similarity in disinfo detector (`backend/disinfo_detector.py`)
- Replaced `MIN_TOKEN_OVERLAP = 3` with `COSINE_THRESHOLD = 0.40`
- Added `_cosine_sim(toks_a, toks_b)` — binary cosine, no sklearn dependency
- `_events_are_same_claim()` now uses cosine similarity (normalizes for event length)

#### EWMA Behavioral Baseline Monitor (`backend/baseline_monitor.py` — NEW)
- `record_event_baseline(source, ...)` — increments hourly bucket count per source
- `compute_source_anomaly(source, ...)` — EWMA + z-score + spike exclusion + 90-obs guard
- `get_all_source_anomalies(...)` — all sources sorted by |z_score|
- DB table: `source_baseline (source, hour_bucket, event_count)`
- Status: NORMAL / EARLY_WARNING / CRITICAL / INSUFFICIENT_BASELINE / UNAVAILABLE

#### ACLED civilian targeting structural rule (`backend/v2_store.py`)
- Removed keyword-based `_CIVILIAN_KEYWORDS` approach
- Added `_ACTOR2_CIVILIAN_TERMS` set (civilians, protesters, aid workers, journalists, etc.)
- Added `_is_civilian_targeted(event, source)` — Actor 2 structural rule; FIRMS=False always
- Added `_firms_collateral_note()` — generates notes field entry for FIRMS events
- Added `VAGUE_ESTIMATE_LOOKUP` — dozens=12, scores=20, hundreds=100 (ACLED sourced minimum)
- `_compute_acled_fields()` now uses `_is_civilian_targeted()` instead of keyword scan

### What is still pending
- Wire `record_event_baseline()` into `ingest_event()` call (1 line in ingestion.py)
- Wire `get_all_source_anomalies()` into health endpoint or new `/api/v2/baseline` endpoint
- 50 manually labelled eval samples (user's task)
